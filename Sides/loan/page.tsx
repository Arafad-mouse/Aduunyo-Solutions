...
import { format, differenceInYears, addYears, subYears } from 'date-fns';
...
// Form Steps
type FormStep = 'personal' | 'employment' | 'loan' | 'documents' | 'review';
...
const loanApplicationSchema = z.object({
  ...
  dateOfBirth: z.date({
    ...
    date => {
      ...
      const maxDate = subYears(today, 100);
  ...
  nationalId: z.string()
    ...
    .max(30, { message: 'National ID cannot exceed 30 characters' }),
  ...
  employmentDuration: z.string()
    .min(1, { message: 'Please select employment duration' })
    .optional(),
  ...
  loanAmount: z.string()
    ...
    .refine(
      val => !isNaN(parseFloat(val)) && parseFloat(val) >= 25,
      { message: 'Minimum loan amount is $25' }
    ),
  ...
  documentNumber: z.string()
    .min(2, { message: 'Document number is required' })
  ...
  reference1Name: z.string()
    ...
    .max(100, { message: 'Reference name cannot exceed 100 characters' }),
    
  reference1Phone: z.string()
    .min(10, { message: 'Reference phone must be at least 10 digits' })
    .regex(phoneRegex, { message: 'Please enter a valid phone number' }),
    
  reference1Relationship: z.string()
    .min(2, { message: 'Please specify your relationship' })
  ...
  data => data.reference1Phone !== data.reference2Phone,
  {
    message: 'Reference phone numbers must be different',
    path: ['reference2Phone'],
  }
).refine(
  data => data.reference1Phone !== data.phoneNumber && data.reference2Phone !== data.phoneNumber,
  {
    message: 'References cannot use your phone number',
...
⏰ Submitted: ${new Date().toLocaleString()}`;
  ...
  const onSubmit = async (formData: FormValues) => {
    ...
    try {
      ...
      if (formData.reference1Phone === formData.phoneNumber) {
      ...
      if (!(dob instanceof Date) || isNaN(dob.getTime())) {
        ...
        setSubmissionMessage('Invalid date of birth');
        setMessageType('error');
        return;
      }
      ...
      const validPaymentPlans = ['Daily', 'Weekly', 'Monthly', '3 months', '6 months', '9 months','1 year'];
